...
const BASE_URL = "http://172.20.172.102:5000";
...
function Gallery() {
  ...
  const handleUpload = async (e) => {
    e.preventDefault();
    if (!selectedFile) return toast.warning("Please choose an image.");
    if (!token) return toast.error("Login as admin first.");
  ...
  const handleSaveEdit = async (id) => {
    ...
    try {
      ...
      setArtworks((prev) =>
        prev.map((a) => (a.id === id ? { ...a, title: newName } : a))
      );
  ...
  return (
    <section style={{ background: "#111", padding: "80px 0" }}>
      <Container>
        <h2 className="text-center text-info mb-4">Art Gallery</h2>
